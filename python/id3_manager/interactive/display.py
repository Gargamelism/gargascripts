...
def show_file_comparison(ui, audio_file: AudioFile) -> None:
    ...
    fields = [
        ...
        (
            ...
            proposed.album_artist if proposed else None,
        ),
        (
            "Track #",
            f"{current.track_number or '?'}/{current.total_tracks or '?'}"
            if current.track_number
            else None,
            f"{proposed.track_number or '?'}/{proposed.total_tracks or '?'}"
            if proposed and proposed.track_number
            else None,
        ),
        (
            "Disc #",
            f"{current.disc_number or '?'}/{current.total_discs or '?'}"
            if current.disc_number
            else None,
            f"{proposed.disc_number or '?'}/{proposed.total_discs or '?'}"
            if proposed and proposed.disc_number
            else None,
        ),
...
def show_progress(ui, current: int, total: int, message: str = "") -> None:
    ...
    print(
        f"\r[{bar}] {pct:5.1f}% ({current}/{total}) {message}\033[K", end="", flush=True
    )
    if current == total:
        print()
...
def show_folder_status(
    ui, folder_path: str, file_count: int, needs_tag_update: int, needs_rename: int
) -> None:
